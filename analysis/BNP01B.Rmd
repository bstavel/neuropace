...
BNP01B_data <- read_csv(path(here(), './data/BNP01B_run1_cleaned_data.csv'), col_types = cols(X1 = col_double(),
...
# clean Biscuits #
BNP01B_data_clean <- BNP01B_data %>%
  mutate(time_step = c(FALSE, diff(Time))) %>%
  mutate(trial_numeric = as.numeric(gsub("Trial_", "", Trial))) %>%
  # convert trials + blocks
  mutate(trials_real = trial_numeric+ (20 * Block)) %>%
  mutate(trial_numeric = trials_real) %>%
  mutate(Trial = paste0("Trial_", trial_numeric)) %>%
  select(-trials_real) %>%
...
# BNP01B_paused_trials <- BNP01B_data_clean %>%
...
BNP01B_data_distance <- BNP01B_data_clean %>%
  filter(Trial != "ITI") %>%
...
BNP01B_all_vars_df <- BNP01B_data_clean %>%
  group_by(subject) %>%
  filter(!is.na(Score)) %>%
  filter(Trial != "ITI") %>%
  filter(Trial != 0) %>%
...
round <- 1
game <- 1
BNP01B_all_vars_df$trial_in_minigame <- 0
BNP01B_all_vars_df$minigame <- 0
for(idx in 1:nrow(BNP01B_all_vars_df)){
  # add to df
  BNP01B_all_vars_df$trial_in_minigame[idx] <- round
  BNP01B_all_vars_df$minigame[idx] <- game
  if(BNP01B_all_vars_df$last_trial_in_minigame[idx] == 1){
    round <- 1
    game <- game + 1
  } else {
    round <- round + 1
  ...
  if(BNP01B_all_vars_df$subject[idx + 1] != BNP01B_all_vars_df$subject[idx] & idx != nrow(BNP01B_all_vars_df)) {
    round <- 1
    game <- 1
  }
...
BNP01B_all_vars_df <- BNP01B_all_vars_df %>%
  group_by(subject) %>%
...
```
```{r create-stim-channels}


start_end_trials_df <- BNP01B_data_clean %>%
  group_by(subject, trial_numeric) %>%
  mutate(begin_stamp = first(EmpTimes)) %>%
  mutate(end_stamp = last(EmpTimes)) %>%
  ungroup() %>%
  select(subject, trial_numeric, EmpTimes, begin_stamp, end_stamp) %>%
  filter( (EmpTimes == begin_stamp) | EmpTimes == end_stamp) %>%
  mutate(trigger = if_else(EmpTimes == begin_stamp, 1, 2)) %>%
  select(-begin_stamp, -end_stamp) %>%
  distinct()
  

write_csv(start_end_trials_df, path(here(), "data", "ieeg_behave", "BNP01B_run1_triggers.csv"))
```
...
no_biscuits_trials <- BNP01B_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
long_trials <- BNP01B_all_vars_df %>%
...
write_csv(bad_df, path(here(), './data/ieeg_behave/BNP01B_bad_trials.csv'))
...
# read in bad df
bad_df <- read_csv(path(here(), './data/ieeg_behave/BNP01B_bad_trials.csv'))


last_away_df <- BNP01B_data_clean %>%
  filter(Trial != "ITI") %>%
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
...
write_csv(last_away_clean_df, path(here(), './data/ieeg_behave/BNP01B_last_away_events.csv'))
