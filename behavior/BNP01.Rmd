...
BNP01_data <- read_csv(path(here(), './data/ieeg_behave/BNP01_run1_cleaned_data.csv'), col_types = cols(X1 = col_double(),
...
# BNP01_paused_trials <- BNP01_data_clean %>%
...
BNP01_data_distance <- BNP01_data_clean %>%
  filter(Trial != "ITI") %>%
...
BNP01_all_vars_df <- BNP01_data_clean %>%
  group_by(subject) %>%
  filter(!is.na(Score)) %>%
  filter(Trial != "ITI") %>%
  filter(Trial != 0) %>%
...
round <- 1
game <- 1
BNP01_all_vars_df$trial_in_minigame <- 0
BNP01_all_vars_df$minigame <- 0
for(idx in 1:nrow(BNP01_all_vars_df)){
  # add to df
  BNP01_all_vars_df$trial_in_minigame[idx] <- round
  BNP01_all_vars_df$minigame[idx] <- game
  if(BNP01_all_vars_df$last_trial_in_minigame[idx] == 1){
    round <- 1
    game <- game + 1
  } else {
    round <- round + 1
  ...
  if(BNP01_all_vars_df$subject[idx + 1] != BNP01_all_vars_df$subject[idx] & idx != nrow(BNP01_all_vars_df)) {
    round <- 1
    game <- 1
  }
...
BNP01_all_vars_df <- BNP01_all_vars_df %>%
  group_by(subject) %>%
...
start_end_trials_df <- BNP01_data_clean %>%
...
write_csv(start_end_trials_df, path(here(), "data", "ieeg_behave", "BNP01_run1_triggers.csv"))
```
...
no_biscuits_trials <- BNP01_data_clean %>%
  filter(Trial != "ITI") %>%
  group_by(trial_numeric) %>%
  mutate(total_eaten = max(Eaten)) %>%
...
write_csv(bad_df, path(here(), './data/ieeg_behave/BNP01_bad_trials.csv'))
...
# read in bad df
bad_df <- read_csv(path(here(), './data/ieeg_behave/BNP01_bad_trials.csv'))


last_away_df <- BNP01_data_clean %>%
  filter(Trial != "ITI") %>%
  filter(trial_numeric != 0) %>%
  mutate(neural_trial_numeric = trial_numeric - 1) %>%
...
write_csv(last_away_clean_df, path(here(), './data/ieeg_behave/BNP01_last_away_events.csv'))
